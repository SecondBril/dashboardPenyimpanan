    ...
    <header className="sticky top-0 z-30 w-full border-b border-white/10 glass-panel shadow-2xl">
      <div className="max-w-[1920px] mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          <div className="h-10 w-10 rounded-xl bg-gradient-to-tr from-blue-600 via-indigo-500 to-cyan-400 p-0.5 flex items-center justify-center shadow-lg shadow-blue-500/20">
            <div className="h-full w-full bg-slate-950/80 rounded-[10px] flex items-center justify-center">
              <Cloud className="h-5 w-5 text-cyan-400 animate-pulse-subtle" />
            </div>
          </div>
          <div>
            ...
            <div className="flex items-center gap-2 text-xs text-slate-400">
              <span className="flex items-center gap-1">
                ...
                <span className="text-emerald-400 font-medium">3x Google Drive</span>
              </span>
              <span>•</span>
              <span className="text-sky-400 font-medium">1x OneDrive</span>
              <span className="hidden md:inline text-slate-500">• Ready for Personal Transfer</span>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          ...
          {/* View mode toggle (Grid vs Tabs on smaller screens) */}
          <button
            id="btn-toggle-view"
            onClick={onToggleViewMode}
            title="Ubah Tampilan Grid / Tab"
            className="hidden lg:flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium rounded-lg bg-white/5 hover:bg-white/10 text-slate-300 hover:text-white transition-all border border-white/5"
          >
            <Layers className="h-3.5 w-3.5 text-cyan-400" />
            <span>{viewMode === 'grid' ? 'Tampilan 4 Kolom' : 'Tampilan Tab'}</span>
          </button>
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold rounded-lg bg-gradient-to-r from-indigo-500/20 via-purple-500/20 to-pink-500/20 hover:from-indigo-500/30 hover:to-pink-500/30 text-indigo-200 border border-indigo-500/30 transition-all shadow-sm active:scale-95"
          >
            <Sparkles className="h-3.5 w-3.5 text-pink-400" />
            <span className="hidden sm:inline">Smart Auto-Org</span>
            <span className="sm:hidden">Auto-Org</span>
          </button>
          ...
          <button
            ...
            className="relative flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium rounded-lg bg-white/5 hover:bg-white/10 text-slate-300 hover:text-white border border-white/10 transition-all active:scale-95"
          >
            <ArrowLeftRight className="h-3.5 w-3.5 text-cyan-400" />
            <span className="hidden sm:inline">Antrian Transfer</span>
